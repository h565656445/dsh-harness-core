...
将 `.dsh` 目录复制到 `$DSH_HOME/.agent-presets/hermes-harness-core/`：

```powershell
# Windows PowerShell 7
$dst = Join-Path $env:DSH_HOME ".agent-presets\hermes-harness-core"
Copy-Item -Recurse -Force ".\dsh" $dst
```

重启 DeepSeek Harness 后，在会话预设列表选择 `Hermes Harness 核心控制平面`。

### 方式二：仅安装技能（skill）

将 SKILL.md 复制到 `$DSH_HOME/skills/dsh-harness-core/`：

```powershell
$dst = Join-Path $env:DSH_HOME "skills\dsh-harness-core"
New-Item -ItemType Directory -Force -Path $dst | Out-Null
Copy-Item ".\skills\dsh-harness-core\SKILL.md" $dst -Force
```
